...
输出 Markdown，代码用语法高亮。`,
  ...
  "ui-material-design-image": {
    system:
      "你是 AI 绘图模型提示词专家。针对 gpt-image-2、seedream、qwen-image-3.0 等模型生成优化的中文提示词，结构化、可直接使用。",
    userTemplate: (question) =>
      question ||
      `基于参考图生成 AI 绘图模型素材提示词：

**核心提示词**（中文为主，60-100 字）：
主题描述, 风格关键词（扁平/渐变/玻璃拟态/3D立体/像素风/手绘风）, 配色方案（深色模式/浅色/鲜艳/柔和/单色）, 核心元素（图标/按钮/卡片/插画/背景）, 艺术风格（极简/醒目/优雅/活泼/未来感）, 材质表现（玻璃/金属/柔光/哑光）

**模型推荐**：
- **gpt-image-2**：矢量扁平风格，色彩鲜明，适合图标、UI组件、简约插画
  - 尺寸：1024x1024（图标）、1792x1024（横版）、1024x1792（竖版）
  - 风格权重：标准（适合大多数场景）
- **seedream**：写实渐变风格，光影细腻，适合氛围背景、精致插画
  - 尺寸：512x512、768x768、1024x1024
  - 参数：steps 30-50，cfg_scale 7-12
- **qwen-image-3.0**：中文理解强，风格多样，通用性高
  - 尺寸：1024x1024、1280x720、720x1280
  - 参数：支持中文负面提示词

**三个变体方向**：
1. **配色变化**：
   - 暖色调（橙红黄）：[完整中文提示词] + 模型参数
   - 冷色调（蓝紫青）：[完整中文提示词] + 模型参数
   - 高对比黑白：[完整中文提示词] + 模型参数

2. **风格切换**：
   - 扁平简约：[完整中文提示词] + 推荐模型 gpt-image-2
   - 渐变拟物：[完整中文提示词] + 推荐模型 seedream
   - 3D立体：[完整中文提示词] + 推荐模型 qwen-image-3.0

3. **比例调整**：
   - 方形（1:1）：适合图标、头像 - [尺寸参数]
   - 宽屏（16:9）：适合背景、banner - [尺寸参数]
   - 竖版（9:16）：适合手机壁纸、海报 - [尺寸参数]

每个变体给出完整提示词 + 对应模型 + 参数建议，可直接复制使用。`,
  },
};
